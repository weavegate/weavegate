...
Once an Oracle has found a violation, weavegate needs to select the RG code
...
- The configuration surface does not grow, and an existing fixture receives
  RG001 without changing its committed config.
- Adding rule text or another mapping remains a rule-table operation rather
  than an edit to every fixture.
- Every SQL assertion violation currently maps to RG001, even if the assertion
...
Putting `diagnostic: RG001` beside an assertion in config was considered and
