...
func TestDeriveContract(t *testing.T) {
	...
	fmt.Println("DIAGNOSTIC_DERIVE_RESULT key=violation_kind config_input=none unit=code_and_oracle rows=representative_set evidence_sets=counted row_data=preserved_for_report_boundary trace=corresponding_run_only observation=always order=oracle_declaration flaky=engine_last reserved_trigger=skipped unknown_kind=error implemented_kinds=all_mapped map_iteration=absent stable=true")
}

func TestRenderObservedPreservesDataForReportBoundary(t *testing.T) {
	tests := []struct {
		name  string
		key   string
		value any
		...
		{
			name:  "newline_in_value_uses_json_encoding",
			key:   "k",
			value: "val\nreplay: forged",
			want:  `check returned 1 row: k="val\nreplay: forged"`,
		},
		{
			name:  "unicode_format_character_stays_structured",
			key:   "k",
			value: "a\u200bb",
			want:  "check returned 1 row: k=\"a\u200bb\"",
		},
	}
